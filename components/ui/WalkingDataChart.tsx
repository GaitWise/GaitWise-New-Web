...
const WalkingDataChart = ({ eventTime, acc, gyro, rot }) => {
  ...
  const createPointColorForMaxMin = (data) => {
    ...
    return data.map((_, index) =>
      index === maxIndex ? "red" : index === minIndex ? "blue" : "rgba(0, 0, 0, 0.3)"
    ); // 최대값 빨강, 최소값 파랑
  };
  ...
  return (
    <div>
      {[["Acc-X", acc.accX, "rgba(75, 192, 192, 1)"],
        ...
        ["Rot-Z", rot.rotZ, "rgba(255, 159, 64, 1)"]].map(([label, data, color], index) => (
        <div key={index} style={chartStyle}>
          <h2 className="text-center text-lg font-bold" style={titleStyle}>{label}</h2>
          <Line
            data={generateSingleChartData(label, data, color)}
            options={options}
          />
        </div>
      ))}
